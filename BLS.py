...
def clean(df):
    ...
    if len(t)<1000:return None,None
...
def get_candidates(t,f):
    ...
    for i in peaks:
        ...
        if len(fine)<10:continue
        rr=bls.power(fine,DURATIONS,objective="likelihood")
        j=int(np.nanargmax(rr.power))
        out.append({"period":float(rr.period[j]),"depth_ppm":float(rr.depth[j]*1e6),"duration_h":float(rr.duration[j]*24),"epoch":float(rr.transit_time[j]),"sde":get_sde(power,i)})
    return sorted(out,key=lambda x:x["sde"],reverse=True)

def harmonic(p1,p2):
    for r in [0.5,1/3,2,3]:
        if abs(p2-p1*r)/(p1*r)<=0.02:return True
    return False
...
def process_star(file):
    ...
    try:
        ...
        t,f=clean(df)
        if t is None:return kepid,None,"CLEAN FAILED"
        cs=get_candidates(t,f)
        best=choose(cs)
        if best is None:return kepid,None,"NO CANDIDATE"
        row={"KIC":kepid,"Period_days":best["period"],"Depth_ppm":best["depth_ppm"],"Duration_hours":best["duration_h"],"Epoch":best["epoch"],"SDE":best["sde"]}
...
def main():
    ...
    print(f"Starting {len(files)} stars with {WORKERS} CPU workers...")
    with ProcessPoolExecutor(max_workers=WORKERS) as executor:
        futures={executor.submit(process_star,file):file for file in files}
        for future in as_completed(futures):
            ...
            if row is not None:
                results.append(row)
                print(f"{done}/{len(files)} KIC_{kepid}: P={row['Period_days']:.3f} d | depth={row['Depth_ppm']:.1f} ppm | dur={row['Duration_hours']:.1f} h | SDE={row['SDE']:.2f}")
            else:
